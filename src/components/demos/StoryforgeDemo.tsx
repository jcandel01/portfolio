...
export function StoryforgeDemo() {
  ...
  return (
    <BrowserFrame url="storyforge.app/project/the-northern-oath">
      <div className="flex h-full bg-demo-story-bg text-white">
        {/* Left: chapters & scenes */}
        <aside className="hidden w-44 shrink-0 flex-col border-r border-white/10 bg-demo-story-raised sm:flex">
          <div className="flex items-center gap-1.5 border-b border-white/10 px-3 py-2.5">
            <BookOpen size={14} className="text-demo-story-accent" />
            <span className="text-caption-strong">The Northern Oath</span>
          </div>
          <div className="no-scrollbar overflow-y-auto py-2">
            {chapters.map((ch) => (
              <div key={ch} className="mb-1">
                <p className="px-3 py-1 text-micro-legal font-semibold uppercase tracking-wide text-demo-story-muted">
                  {ch}
                </p>
                {scenes
                  ...
                  .map((s) => (
                    <button
                      ...
                      className={`flex w-full items-center gap-1.5 px-3 py-1.5 text-left text-fine-print transition-colors ${
                        s.id === activeId
                          ? 'bg-demo-story-accent/20 text-white'
                          : 'text-demo-story-muted active:bg-white/5'
                      }`}
                    >
        ...
        <main className="no-scrollbar flex-1 overflow-y-auto px-5 py-4">
          <p className="text-micro-legal uppercase tracking-widest text-demo-story-muted">{active.chapter}</p>
          <h3 className="mb-3 text-tagline font-display text-white">{active.title}</h3>
          {active.body.split('\n\n').map((para, i) => (
            <p key={i} className="mb-3 text-caption leading-relaxed text-white/80">
              {para}
            </p>
          ))}
          <div className="mt-4 flex flex-wrap gap-1.5">
            {active.characters.map((c) => (
              <span
                key={c}
                className="inline-flex items-center gap-1 rounded-full bg-demo-story-accent/15 px-2 py-0.5 text-micro-legal text-demo-story-accent"
              >
                <Users size={9} /> {c}
              </span>
            ))}
        ...
        {/* Right: timeline */}
        <aside className="hidden w-44 shrink-0 flex-col border-l border-white/10 bg-demo-story-raised lg:flex">
          <div className="flex items-center gap-1.5 border-b border-white/10 px-3 py-2.5">
            <Clock size={14} className="text-demo-story-accent" />
            <span className="text-caption-strong">Timeline</span>
          </div>
                ...
                <div key={e.id} className="relative pl-4">
                  <span
                    className={`absolute left-0 top-1.5 h-2 w-2 rounded-full ${
                      inScene ? 'bg-demo-story-accent' : 'bg-white/20'
                    }`}
                  />
                  <span className="absolute left-[3px] top-3 h-full w-px bg-white/10" />
                  <div className={`mb-2.5 ${inScene ? '' : 'opacity-50'}`}>
                    <p className="text-micro-legal text-demo-story-muted">{e.day}</p>
                    <p className="flex items-center gap-0.5 text-fine-print text-white/80">
                      {inScene && <ChevronRight size={10} className="text-demo-story-accent" />}
                      {e.label}
                    </p>
                  </div>
                </div>
